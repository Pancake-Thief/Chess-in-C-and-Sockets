...
void ProcessRequest(int DataSocketFD, char RecvBuf[256], int n){
	...
	if(0 == strncmp(RecvBuf, "move", 4) || 0 == strncmp(RecvBuf, "chat", 4)){
        ...
        //compare username and password to see if it exists in hash table
        //if login info matches with an account already in the hash table: 
	...
	if(login(username, password) == 1){
        ...
        //else if login info doesn't match:
            //strncpy(SendBuf, "Invalid info", sizeof(SendBuf)-1);
